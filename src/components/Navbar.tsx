...
export default function Navbar() {
  ...
  const navigate = (label: string, href: string, e: React.MouseEvent) => {
    setActive(label);
    setOpen(false);
  ...
  return (
    <header className={cn(
      ...
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <a href="#home" onClick={(e) => navigate("Home", "#home", e)}>
        ...
        <nav className="hidden md:flex items-center bg-zinc-100/80 dark:bg-zinc-900/80 backdrop-blur rounded-full px-1.5 py-1 border border-zinc-200 dark:border-zinc-800">
          {nav.map(({ label, href }) => (
            <a
              key={label}
              href={href}
              onClick={(e) => navigate(label, href, e)}
              className={cn(
            ...
            {nav.map(({ label, href }) => (
              <a
                ...
                className={cn(
                  ...
                  active === label ? "text-[#0047FF]" : "text-zinc-600 dark:text-zinc-400"
                )}
              >
                {label}
              </a>
